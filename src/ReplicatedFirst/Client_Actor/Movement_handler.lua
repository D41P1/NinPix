...
function Movement_Handler:Walk(Character: Model, Event: RemoteEvent?, Key: string)
    if RateLimiter.ClientCheck(PlayerName, "Move") then  return end
    if not  Movement_Handler[Key] then 
        local StopEvent:any = Events.StopWalk;
        Movement_Handler["Stop"](Character, StopEvent, "StopWalk"); 
        Movement_Handler.CheckPlayerCentrePosNetwork()
        return
    end
    local Dir  = MovementHelper:GiveDirection(Character, Key)
    Movement_Handler[Key](Character, Event, "StartWalk", Dir)     
end
function Movement_Handler:Jump(Character: Model, Event: RemoteEvent?, Key: string)
